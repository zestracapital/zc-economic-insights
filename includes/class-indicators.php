    ...
    class ZC_DMT_Indicators {
        ...
        public static function get_data_by_slug($slug, $start = null, $end = null) {
            ...
            if (isset($indicator->source_type)) {
                // Handle calculation indicators
                if ($indicator->source_type === 'calculation' && class_exists('ZC_DMT_Calculations')) {
                    return self::get_calculation_data($indicator, $start, $end);
                } elseif ($indicator->source_type === 'google_sheets' && class_exists('ZC_DMT_DataSource_Google_Sheets')) {
        ...
        /**
         * Get calculation data for calculation indicators
         */
        private static function get_calculation_data($indicator, $start = null, $end = null) {
            // Parse source_config to get calculation info
            $source_config = json_decode($indicator->source_config, true);
            if (!$source_config || !isset($source_config['calculation_id'])) {
                return new WP_Error('invalid_calculation', __('Invalid calculation configuration', 'zc-dmt'));
            }

            $calculation_id = intval($source_config['calculation_id']);
            $calculation = ZC_DMT_Calculations::get_calculation($calculation_id);
            
            if (!$calculation) {
                return new WP_Error('calculation_not_found', __('Calculation not found', 'zc-dmt'));
            }

            // Execute the calculation to get current results
            $result = ZC_DMT_Calculations::execute_calculation($calculation_id);
            
            if (is_wp_error($result)) {
                return $result;
            }

            // Format result as series data
            $series = array();
            if (isset($result['data']) && is_array($result['data'])) {
                foreach ($result['data'] as $point) {
                    if (is_array($point) && count($point) >= 2) {
                        ...
                        if ($start && strtotime($date) < strtotime($start)) continue;
                        if ($end && strtotime($date) > strtotime($end)) continue;
            ...
            if (empty($series)) {
                return new WP_Error('no_calculation_data', __('No data generated from calculation', 'zc-dmt'));
            }
