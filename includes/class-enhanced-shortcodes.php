...
if (!class_exists('ZC_DMT_Enhanced_Shortcodes')) {
    ...
    class ZC_DMT_Enhanced_Shortcodes {
        ...
        /**
         * Register enhanced and secure shortcodes
         */
        public static function register() {
            add_shortcode('zc_economic_dashboard', array(__CLASS__, 'render_secure_dashboard'));
            add_shortcode('zc_chart_enhanced', array(__CLASS__, 'render_secure_chart'));
            add_shortcode('zc_chart_comparison', array(__CLASS__, 'render_secure_comparison'));
            add_shortcode('zc_chart_calculation', array(__CLASS__, 'render_secure_calculation'));
        ...
        /**
         * Render secure economic dashboard with enhanced protection
         */
        public static function render_secure_dashboard($atts) {
            $atts = shortcode_atts(array(
                'mode' => 'dynamic',
                'height' => '600',
                'show_header' => 'true',
                'show_search' => 'true',
                'show_comparison' => 'false', // Disabled by default for security
                'show_timeframes' => 'true',
                'show_chart_types' => 'true',
                'show_stats' => 'true',
                'show_fullscreen' => 'false', // Disabled by default for security
            ...
            $container_id = 'zc-secure-dashboard-' . wp_generate_uuid4();
            ...
            <div id="<?php echo esc_attr($container_id); ?>" 
                 class="zc-secure-dashboard-container <?php echo esc_attr($sanitized_config['class']); ?>"
                 data-config="<?php echo esc_attr($config_json); ?>"
                 data-access-key="<?php echo esc_attr($sanitized_config['access_key']); ?>"
                 style="min-height: <?php echo esc_attr($sanitized_config['height']); ?>px;">
                
                <div class="zc-dashboard-loading">
                    <div class="loading-spinner"></div>
                    <div class="loading-content">
                        <span class="loading-title"><?php esc_html_e('Zestra Capital', 'zc-dmt'); ?></span>
                        <span class="loading-subtitle"><?php esc_html_e('Loading Secure Dashboard...', 'zc-dmt'); ?></span>
                    </div>
                </div>
            ...
            (function() {
                ...
                function initSecureDashboard() {
                    if (typeof window.ZCSecureDashboard !== 'undefined' && window.ZCSecureDashboard.init) {
                        try {
                            ...
                            if (container) {
                                ...
                                const instance = window.ZCSecureDashboard.init('<?php echo esc_js($container_id); ?>', config);
                                if (!instance) {
                                    throw new Error('Secure dashboard initialization failed');
                                }
                                
                                console.log('Secure dashboard initialized:', '<?php echo esc_js($container_id); ?>');
                            }
                        } catch (error) {
                            console.error('Secure dashboard initialization error:', error);
                            showErrorFallback('<?php echo esc_js($container_id); ?>');
                        }
                    } else {
        ...
        /**
         * Render secure enhanced chart
         */
        public static function render_secure_chart($atts) {
            $atts = shortcode_atts(array(
                'id' => '',
                'type' => 'line',
                'height' => '600',
            ...
            $static_config = array(
                'mode' => 'static',
                'height' => max(300, min(1200, intval($atts['height']))),
                'show_header' => 'false', // Simplified for static charts
                'show_search' => 'false',
            ...
            return self::render_secure_dashboard($static_config);
        }

        /**
         * Render secure comparison chart
         */
        public static function render_secure_comparison($atts) {
            $atts = shortcode_atts(array(
                'indicators' => '',
                'height' => '600',
                'chart_type' => 'line',
            ...
            $indicator_slugs = array_filter($indicator_slugs);
            
            // Security: Limit number of indicators in comparison
            if (count($indicator_slugs) > 3) { // Reduced limit for security
                $indicator_slugs = array_slice($indicator_slugs, 0, 3);
            }
            ...
            return self::render_secure_dashboard($dashboard_config);
        }

        /**
         * Render secure calculation chart
         */
        public static function render_secure_calculation($atts) {
            // Calculations feature currently disabled for security
            return self::error_box(__('Calculations feature is currently disabled for security reasons.', 'zc-dmt'));
        }
        ...
        private static function validate_shortcode_security($atts) {
            ...
            if (self::is_ip_blocked($ip)) {
                self::log_security_event('blocked_ip_shortcode_access', array('ip' => $ip));
                return false;
            }

            // Basic rate limiting for shortcode rendering
            ...
            if (!empty($access_key)) {
                if (!self::validate_access_key($access_key)) {
                    self::log_security_event('invalid_access_key_shortcode', array(
        ...
        private static function check_shortcode_rate_limit() {
            ...
            // Allow 20 shortcode renders per hour per IP/user (reduced for security)
            if ($requests >= 20) {
                self::log_security_event('shortcode_rate_limit_exceeded', array(
                    'ip' => $ip,
                    'user_id' => $user_id,
                    'requests' => $requests
                ));
        ...
        private static function sanitize_dashboard_config($atts) {
            ...
            $sanitized['height'] = max(300, min(1200, intval($atts['height'])));
            ...
            $sanitized['theme'] = in_array($atts['theme'], ['light', 'dark']) ? $atts['theme'] : 'light';
            
            // Sanitize text fields with length limits
            $sanitized['title'] = substr(sanitize_text_field($atts['title']), 0, 100);
            $sanitized['description'] = substr(sanitize_text_field($atts['description']), 0, 200);
        ...
        private static function sanitize_access_key($key) {
            ...
            // Validate format (alphanumeric and certain special characters only)
        ...
        private static function enqueue_secure_dashboard_assets() {
            ...
            if (wp_script_is('zc-secure-dashboard', 'enqueued')) {
                return;
            }

            // Cache-busting versions
            $css_path = ZC_DMT_DIR . 'assets/css/secure-dashboard.css';
            $js_path = ZC_DMT_DIR . 'assets/js/secure-dashboard.js';
            ...
            wp_enqueue_style(
                'zc-secure-dashboard',
                ZC_DMT_URL . 'assets/css/secure-dashboard.css',
                array(),
                $css_ver
            );
            ...
            wp_enqueue_script(
                'zc-secure-dashboard',
                ZC_DMT_URL . 'assets/js/secure-dashboard.js',
                array('chartjs', 'chartjs-adapter'),
                $js_ver,
                true
            );

            // Secure localized data (MINIMAL EXPOSURE)
            $localized_data = array(
                ...
                'security' => array(
                    ...
                    'maxIndicators' => 3, // Reduced for security
                    'maxSeriesPoints' => 5000
                ),
                'features' => array(
                    'calculationsEnabled' => false, // Disabled for security
                    'comparisonEnabled' => false, // Disabled by default
                    'searchEnabled' => true,
                    'themeToggleEnabled' => true
                )
            );

            wp_localize_script('zc-secure-dashboard', 'zcDmtConfig', $localized_data);
        }
        ...
        private static function get_safe_indicators_list() {
            ...
            $indicators = $wpdb->get_results(
                "SELECT id, name, slug 
                 ...
                 WHERE is_active = 1 
                 ORDER BY name ASC 
                 LIMIT 50" // Security limit reduced
            );
        ...
        private static function error_box($message) {
            return '<div class="zc-chart-error" role="alert">'
                ...
                . '<style>'
                . '.zc-chart-error { display: flex; align-items: center; padding: 16px; border: 1px solid #fed7d7; border-radius: 8px; background: #fef5e7; color: #744210; font-family: sans-serif; margin: 16px 0; }'
                . '.zc-chart-error .error-icon { font-size: 1.25rem; margin-right: 12px; flex-shrink: 0; }'
                . '.zc-chart-error .error-message { font-size: 0.85rem; font-weight: 500; }'
                . '</style>'
                . '</div>';
        }
        ...
        private static function is_ip_blocked($ip) {
            $blocked_ips = get_option('zc_dmt_blocked_ips', array());
